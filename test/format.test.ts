...
describe("formatAgentLine", () => {
  ...
  it("falls back to the kind, then to no agent", () => {
    expect(formatAgentLine(EN, agent())).toBe("● **w6:p1** claude · working");
    expect(formatAgentLine(EN, agent({ agent: null, agent_status: "unknown" }))).toBe("? **w6:p1** no agent · unknown");
  });
  ...
  it("flattens newlines injected through the title", () => {
    const line = formatAgentLine(EN, agent({ terminal_title_stripped: "evil\n**not a real line**" }));
    expect(line.split("\n")).toHaveLength(1);
  });
});
...
describe("formatAgentLine with a tab label", () => {
  it("leads with the label and keeps the pane id as the secondary ref", () => {
    expect(formatAgentLine(EN, agent({ tab_label: "sample#reviewer", agent_status: "idle" }))).toBe(
      "○ **sample#reviewer** claude · w6:p1 · idle",
    );
  });
  it("prefers the Herdr agent name over the tab label", () => {
    expect(formatAgentLine(EN, agent({ name: "reviewer", tab_label: "sample#reviewer" }))).toBe(
      "● **reviewer** claude · w6:p1 · working",
    );
  });
  it("qualifies both the name and the pane ref on a machine", () => {
    expect(formatAgentLine(EN, agent({ tab_label: "sample#builder" }), "w6:p1@buildbox", "buildbox")).toBe(
  ...
  it("keeps a hostile label on one bounded line that cannot close the bold", () => {
    const line = formatAgentLine(EN, agent({ tab_label: "a**b`c\nd" + "x".repeat(200) }));
    expect(line.split("\n")).toHaveLength(1);
  ...
  it("names the label in the send receipt", () => {
    expect(formatSendAccepted(EN, agent({ tab_label: "sample#reviewer" }), "run the tests", "off")).toContain(
      "Sent to **w6:p1** (sample#reviewer).",
    );
  });
  it("groups a machine's labelled agents with qualified refs", () => {
    const out = formatServerList(EN, 
      [
...
describe("formatAgentList", () => {
  it("explains an empty herd", () => {
    expect(formatAgentList(EN, [], [])).toContain("no running coding agent");
    expect(formatAgentList(EN, [agent({ agent: null })], [])).toContain("no running coding agent");
  });
  it("lists live agents only and marks the watched ones", () => {
    const out = formatAgentList(EN, [agent(), agent({ pane_id: "w6:p2", agent: null }), agent({ pane_id: "w6:p3" })], [
      watch({ paneId: "w6:p3" }),
    ]);
...
describe("formatServerList", () => {
  ...
  it("marks a watch on the right server only", () => {
    ...
    const out = formatServerList(EN, groups, [watch({ serverId: "local", paneId: "w1:p1" })]);
    const lines = out.split("\n");
  ...
  it("says when a server has nothing running", () => {
    const out = formatServerList(EN, [{ ...local, agents: [] }, { ...buildbox, agents: [] }], []);
    ...
    expect(formatServerList(EN, [{ ...local, agents: [] }], [])).toContain("Start claude or codex");
  });

  it("tells the operator what to check when the local server is down", () => {
    const out = formatServerList(EN, [{ ...local, down: "ENOENT" }, { ...buildbox, agents: [agent()] }], []);
  ...
  it("appends a note about the machine list", () => {
    const out = formatServerList(EN, [{ ...local, agents: [agent()] }], [], "Machine list unavailable: herdr not found");
...
describe("formatSendAccepted", () => {
  it("names the pane, the watch state and the prompt", () => {
    expect(formatSendAccepted(EN, agent({ name: "reviewer" }), "run  the tests", "watching")).toBe(
      ["Sent to **w6:p1** (reviewer).", "I will tell you when it finishes or needs input.", "> run the tests"].join("\n"),
    );
    expect(formatSendAccepted(EN, agent(), "go", "off")).toContain("Not watching");
  });
});

describe("formatStatus", () => {
  it("adds the watch age and the pane tail", () => {
    const out = formatStatus(EN, agent(), "working on it\n", watch());
    expect(out).toContain("watching since 2026-01-01T00:00:00.000Z");
    expect(out).toContain("```\nworking on it\n```");
  });
  it("omits the block when there is no output", () => {
    expect(formatStatus(EN, agent(), undefined, undefined)).toBe("● **w6:p1** claude · working");
  });
});

describe("formatNotification", () => {
  it("announces the settled states", () => {
    expect(formatNotification(EN, watch(), "done", undefined)).toContain("finished");
    expect(formatNotification(EN, watch(), "idle", undefined)).toContain("finished");
    expect(formatNotification(EN, watch(), "exited", undefined)).toContain("exited");
    expect(formatNotification(EN, watch(), "timed_out", undefined)).toContain("watch deadline");
    expect(formatNotification(EN, watch(), "done", undefined)).toContain("> run the tests");
  });
  it("tells the truth about answering a blocked prompt", () => {
    const out = formatNotification(EN, watch(), "blocked", "Do you want to proceed?\n");
  ...
  it("adds the advice only when blocked", () => {
    expect(formatNotification(EN, watch(), "done", "all green\n")).not.toContain("Answer it in the terminal");
  });
